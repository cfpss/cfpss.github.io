function total_leaf = Paper_modified_3_A(newBR)
...
trunk = face(1:192,:);
%trisurf(trunk,obj.v(:,1)*0.5,obj.v(:,3)*0.5,obj.v(:,2),'FaceColor',[0.26,0.33,1.0 ]);
trisurf(trunk,obj.v(:,1)*0.5,obj.v(:,3)*0.5,obj.v(:,2),'FaceColor',[0.2,0,0 ]);
%%%%%%%%%%%%%%%%%%%%%%%%%%% Branch %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%for i=3073:3863
face = [];
for i=193:312
...
while i <= length(lsys_branch(:,1))
    ...
    rotation = rotation + rand_a +  newangle(i) - tmp_angle;
    
    
    trisurf(face,...
        (cos(rotation)*obj.v(:,1)+sin(rotation)*obj.v(:,3))*(1/(tmp_i*ns + 1)),...
        (-sin(rotation)*obj.v(:,1)+cos(rotation)*obj.v(:,3))*(1/(tmp_i*ns + 1)),...
...
while i <= length(lsys_branch(:,1))
    ...
    count3 = count3 + 1;
    rotation1 = rotation1 + rand_a + newangle(i) - tmp_angle;
    
    tmp_angle = newangle(i);
    i = i+1;
    i2 = i;
    
    rotation2 = rotation1 + rand_a + newangle(i) - tmp_angle;
    ...
    for L = 20793:21255
        ...
        leafs(j,:) = [(cos(rotation1)*obj.v(L,1)+sin(rotation1)*obj.v(L,3))*(1/(tmp_i*ns + 1)),...
            (-sin(rotation1)*obj.v(L,1)+cos(rotation1)*obj.v(L,3))*(1/(tmp_i*ns + 1)),...
            (tmpz(L)+lsys_branch(i1,4)*3)];
        leafs_norm (j,:) = obj.vn(L,:);
        total_leaf = [total_leaf;[leafs(j,:), leafs_norm(j,:)]];
        j = j+1;
        ...
        leafs(j,:) = [(cos(rotation2)*obj.v(L,1)+sin(rotation2)*obj.v(L,3))*(1/(tmp_i*ns + 1)),...
            (-sin(rotation2)*obj.v(L,1)+cos(rotation2)*obj.v(L,3))*(1/(tmp_i*ns + 1)),...
