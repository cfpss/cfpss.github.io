...
%newLeaf = modified(); 
%newLeaf = modified_1(); 
[newLeaf,xn,yn,radius] = modified_2(newBR); 
...
% hold off;
    ...
    for mit = 1:length(impf)
        imp = impf{mit};
    
    figure()

    %plot((1:24000)./400E3*1E3,imp/max(imp),'-k');
    plot((1:n_sample(mit))./400E3*1E3,imp/max(imp),'-k');
    xlim([0,35])
    ylim([-1,1])
