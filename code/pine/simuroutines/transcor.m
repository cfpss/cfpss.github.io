...
newLeafcenM = repmat(LeafcenM, 1,1,5) + ones(n_leaf,3,5).*(-1 + 2*rand([n_leaf,3,5]))*5E-3;

newLeafcenM2 = [newLeafcenM(:,:,1); newLeafcenM(:,:,2); newLeafcenM(:,:,3); newLeafcenM(:,:,4); newLeafcenM(:,:,5)];

Transmatrix = repmat([-x_sonar -y_sonar -z_sonar],5*n_leaf,1);
Leafcentrans = newLeafcenM2 + Transmatrix; % Transmatrix is 0 matrix here.
