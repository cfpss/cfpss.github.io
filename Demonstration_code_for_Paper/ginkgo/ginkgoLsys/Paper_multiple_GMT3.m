...
function [BR, lf, ntip, Itip] = Paper_multiple_GMT3(nReps, bRatio, angle, binit, lfnode)
...
% 
%  all_rules = ["'d(d)+d)'";"'d(d)+d)[d(d)+d)'";"'d(d)+d)[d(d)+d)[d(d)+d)'";"'d(d)+d)[d(d)+d)[d(d)+d)[d(d)+d)'";...
...
% 
% 
%all_rules = ["'d(g)['";"'d(g)[g)+g)'";"'d(g)+[d(g)+[d(g)'";...
 %   "'d(g)+[d(g)+[d(g)+[d(g)'";];
...
%             vNg = vNg +1;
        ...
        case '('
           ...
           if xT == 0 & yT == 0
               stack(stkPtr).zT = zT + 0.2 + rand(1)/2;
           end
            stack(stkPtr).u = u ;
            stack(stkPtr).v = v ;
            stack(stkPtr).w = w ;
